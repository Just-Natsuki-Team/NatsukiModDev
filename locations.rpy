    ...
    class Location(object):
        ...
        def __init__(
            ...
            if not os.path.isdir(renpy.config.gamedir + "/mod_assets/backgrounds/{0}".format(image_dir)):
                raise Exception("[ERROR]: Image dir '{0}' is not a directory.".format(image_dir))
            ...
            if not renpy.loadable(day_image_fp):
                raise Exception("[ERROR]: Day image ('{0}/{1}') is not loadable.".format(day_image_fp))

            #And the night
            if not renpy.loadable(night_image_fp):
                raise Exception("[ERROR]: Night image ('{0}/{1}') is not loadable.".format(night_image_fp))
    ...
    class Room(object):
        ...
        def is_day(self):
            ...
            OUT:
                ...
                False if night
            """
            return self.sunrise <= datetime.datetime.now().time() < sunset
        ...
        def is_showing_day_room(self):
            """
            Checks if we're showing the night room
            """
            return renpy.showing(self.location.day_image)
