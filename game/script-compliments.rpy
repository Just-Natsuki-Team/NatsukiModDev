...
label compliment_thoughtful:
    ...
    if jn_compliments.last_compliment_type == jn_compliments.JNComplimentTypes.thoughtful:
        if Natsuki.isEnamored(higher=True):
            n  "...Wow.{w=0.75}{nw}"
            extend  " You...{w=0.75}{nw}"
            extend  " really are still hung up over that, huh?"
            n  "But seriously,{w=0.2} [player] -{w=0.5}{nw}"
            extend  " don't worry about it!{w=0.75}{nw}"
            extend  " It's totally fine."
            n  "Besides,{w=0.75}{nw}"
            $ friend_type = "girlfriend" if Natsuki.isLove(higher=True) else "friend"
            extend  " what kind of [friend_type] would I be if I {i}wasn't{/i} at least trying to be thoughtful?"
            n  "P-{w=0.2}plus it looks like you're trying just as much anyway!"
            n  "..."
            n  "Well,{w=0.5}{nw}" 
            extend  " if it puts your mind at ease...{w=1}{nw}"
            $ chosen_descriptor = jn_utils.getRandomEndearment() if Natsuki.isLove(higher=True) else jn_utils.getRandomTease()
            extend  " no worries,{w=0.2} [chosen_descriptor]!"
            n  "You don't have to ask me to keep it up!"

        else:
            n  "H-{w=0.2}huh?{w=0.75}{nw}"
            extend  " Oh,{w=0.2} right.{w=0.5}{nw}"
            extend  " Ahaha."
            n  "Well,{w=0.5}{nw}"
            extend  " just like I was saying before -{w=0.5}{nw}"
            extend  " it's really {i}not{/i} much to ask at all."
            n  "...So don't worry about it,{w=0.2} you goof!"
            n  "It's no sweat off my back!"
