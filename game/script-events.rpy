...
label holiday_christmas_day:
    ...
    n  "It's here!{w=0.75}{nw}"
    extend  " It's actually,{w=0.5}{nw}"
    extend  " finally,{w=0.5}{nw}"
    ...
    if jn_get_current_time_block in [JNTimeBlocks.early_morning,{w=0.2} JNTimeBlocks.mid_morning,{w=0.2} JNTimeBlocks.late_morning]:
        n  "Come on!{w=0.75}{nw}"
        extend  " Get the sleep out of your eyes already,{w=0.2} [player]!"
        n  "Up and at 'em!{w=0.5}{nw}"
        extend  " C'mon,{w=0.2} [player]!"

    else:
        n  "What even {i}took{/i} you so long?{w=0.75}{nw}"
        extend  " Did you forget what day it was or something?{w=0.5}{nw}"
        extend  " It's time to celebrate!"
        n  "'Cause..."

    n  "IT'S CHRISTMAAAAS!{w=1}{nw}"
    extend  " Ehehe!"
    n  "Man,{w=0.75}{nw}"
    extend  " it feels so good to {i}finally{/i} say that..."
    n  "I mean...{w=0.75}{nw}"
    extend  " it isn't as if I had tons planned or anything.{w=1}{nw}"
    extend  " ...Not like there's much {i}to{/i} plan here."
    n  "But there's just something about Christmas that brings that sense of relief,{w=0.2} you know?"
    n  "Studies can take a hike,{w=0.75}{nw}"
    extend  " everything's all arranged and ready to go for everyone..."
    n  "And even if it's just for a couple days..."
    n  "Just having all that weight and stress removed {i}rocks{/i}!{w=1}{nw}"
    extend  " It's great!"
    n  "Like I can just feel the stress of the year just washing away from me..."
    n  "And I don't even have to cook anything here!{w=0.75}{nw}"
    extend  " Ehehe..."
    n  "..."
    n  "..."
    n  "It's...{w=0.75}{nw}"
    extend  " rough sometimes,{w=1}{nw}"
    extend  " you know."
    n  "Christmas,{w=0.2} I mean."
    n  "..."
    n  "..."
    n  "...How do I {i}even{/i} put this..."
    n  "..."
    n  "W-{w=0.2}we were always 'traditional',{w=0.2} my family.{w=1}{nw}"
    extend  " I-{w=0.2}if we were asked."
    n  "...Heh.{w=1}{nw}"
    extend  " Why,{w=0.2} you wonder?"
    n  "...Think about it,{w=0.2} [player]."
    n  "..."
    n  "...You don't need to buy gifts,{w=0.2} if you're {i}traditional{/i}.{w=1.25}{nw}"
    extend  " You don't need to invite guests,{w=0.2} if you're {i}traditional{/i}."
    n  "..."
    n  "You see where I'm going with this...{w=1}{nw}" 
    extend  " right?"
    n  "N-{w=0.2}not celebrating it wasn't a {i}choice{/i} at my place,{w=0.2} [player]."
    n  "..."
    n  "So..."
    n  "...I made my own.{w=0.75}{nw}" 
    extend  " I'd sneak out."
    n  "Heh.{w=0.75}{nw}"
    extend  " I'd already gotten {i}real{/i} good at figuring out where the squeaky floorboards were,{w=0.5}{nw}"
    extend  " I'll tell you that much."
    n  "Not like they particularly cared where I was..."
    n  "..."
    n  "But my friends always did.{w=1}{nw}"
    extend  " We'd pre-arrange it all.{w=1}{nw}"
    extend  " Just before the winter break."
    n  "Sayori's place,{w=0.75}{nw}"
    extend  " Monika's...{w=1}{nw}"
    extend  " It honestly didn't even matter."
    n  "...Wherever I went?{w=1}{nw}"
    extend  " So long as we were all together?"
    n  "..."
    n  "T-{w=0.3}that's where {i}my{/i} home was."
    n  "I didn't even care what I got."
    n  "It didn't matter.{w=1}{nw}"
    extend  " N-{w=0.2}not really."
    n  "Just...{w=1}{nw}"
    extend  " warmth.{w=0.5} P-{w=0.2}people who really {i}cared{/i}."
    n  "N-{w=0.2}not about money.{w=1}{nw}"
    extend  " About me.{w=1}{nw}"
    extend  " Even if I could never get them anything..."
    n  "...That was a gift enough to me."
    n  "So that's why..."
    n  "S-{w=0.3}so that's..."
    n  "...t-{w=0.3}that's...{w=1}{nw}"
    extend  " w-{w=0.3}why..."
    n  "..."
    ...
    n  "I-{w=0.3}I'm fine.{w=1}{nw}"
    extend  " I'm fine!"
    n  "I-{w=0.2}it's just that..."
    n  "..."
    n  "..."
    n  "...They aren't here anymore,{w=0.2} [player].{w=1.25}{nw}"
    extend  " They haven't been here a long time now."
    n  "...T-they're gone."
    n  "But they never stopped being my friends.{w=1.25}{nw}"
    extend  " A-{w=0.2}and I guess that's why I'm still celebrating."
    n  "...For them."

    if Natsuki.isEnamored(higher=True):
        n  "..."
        n  "...A-{w=0.3}and for you."

    n  "So..."
    n  "..."
    n  "...Thanks,{w=0.2} [player].{w=1}{nw}"
    extend  " Really."
    n  "I don't have {i}all{/i} my friends right now,{w=0.75}{nw}" 
    extend  " but..."

    if Natsuki.isLove(higher=True):
        n  "...Just having you here,{w=0.2} [player]?"
        n  "Heh."
        n  "...Yeah.{w=1}{nw}"
        ...
        n  "...A-{w=0.2}at least I got my best one."

    else:
        n  "I think..."
        n  "..."
        n  "I-{w=0.2}I think even just the one here is enough right now."
    ...
    if christmas_poem:
        ...
        n  "..."
        n  "...I did get you something,{w=0.2} you know."
        n  "..."
        n  "H-{w=0.3}hey!{w=0.75}{nw}"
        extend  " Don't give me that look."
        n  "You didn't {i}seriously{/i} think all I had to give you was a {i}story{/i},{w=0.2} did you?"
        n  "I had to at least {i}try{/i},{w=0.75}{nw}"
        extend  " s-so..."

        if Natsuki.isEnamored(higher=True):
            $ chosen_tease = jn_utils.getRandomTease()
            n  "..."
            n  "...J-{w=0.2}just look at it already,{w=0.2} [chosen_tease]."

        else:
            n  "..."
            n  "Nnnnnnn-!"
            n  "...J-{w=0.2}just...{w=1}{nw}" 
            extend " read it already,{w=0.2} [player].{w=1.25}{nw}"
