...
label talk_using_computers_healthily:
    if get_topic("talk_using_computers_healthily").shown_count > 0:
        n 1tslsl "..."
        n 1cslpu "...Huh."
        n 1tnmaj "You know,{w=0.2} [player]...{w=1}{nw}"
        extend 1tlraj " I just thought of something.{w=0.75}{nw}"
        extend 1unmbo " About how you actually visit me and all."
        n 1ulraj "So...{w=1}{nw}"
        extend 1tlrfl " you gotta be at your desk to actually talk to me,{w=0.5}{nw}"
        extend 1tnmca " right?{w=0.75}{nw}"
        extend 1cslss " Or using some kind of computer at least."
        n 1tslfl "And you've been here for a bunch of time already too,{w=0.2} now that I think about it."

        if Natsuki.isEnamored(higher=True):
            n 1unmfll "N-{w=0.2}not that I'm saying I don't appreciate it or anything like that!{w=0.75}{nw}"
            extend 1ccsgslsbr " O-{w=0.2}of course I do!{w=0.75}{nw}"
            $ chosen_tease = jn_utils.getRandomTease()
            extend 1csrbolsbr " You should really know that by now anyway,{w=0.2} [chosen_tease]."
            n 1clraj "Though..."

        elif Natsuki.isAffectionate(higher=True):
            n 1unmeml "N-{w=0.2}not that it's a problem,{w=0.5}{nw}" 
            extend 1clrwrlsbl " or anything like that!{w=0.75}{nw}"
            extend 1ccspolsbl " Y-{w=0.2}you should know it isn't by now anyway,{w=0.2} [player]."
            n 1clrpu "Though..."

        else:
            n 1unmeml "N-{w=0.2}not that I'm saying it's some kind of problem or anything like that,{w=0.5}{nw}" 
            extend 1ccsgslsbl " obviously!{w=0.75}{nw}"
            extend 1ccspolsbl " O-{w=0.2}of course not!"
            n 1csrbolsbl "..."
            n 1clrpu "Though..."

        n  "Doesn't that mean you're spending a whole bunch of extra time sat around with your computer every day?{w=0.75}{nw}"
        ...
        if (jn_activity.ACTIVITY_MANAGER.hasPlayerDoneActivity(jn_activity.JNActivities.work_applications)
            ...
            $ activity_mention = random.choice(["drawing",{w=0.2} "programming",{w=0.2} "working"])
            n  "I mean,{w=0.2} don't get me wrong -{w=0.5}{nw}"
            extend  " I know you use your computer already for a bunch of stuff.{w=0.75}{nw}"
            extend  " Like [activity_mention]!"

        else:
            n  "I mean...{w=1}{nw}"
            extend  " I guess you probably still use your computer for other stuff already,{w=0.2} right?"

        n  "But that still doesn't change the fact you're gonna be racking up a load of extra screen time thanks to me."
        
        if Natsuki.isEnamored(higher=True):
            n  "And the last thing I wanna hear about is you getting yourself all cramped up because you were all slouched over like a potato for hours.{w=0.75}{nw}"
            $ chosen_tease = jn_utils.getRandomTease().capitalize()
            extend  " [chosen_tease]."

        else:
            n  "And the last thing I wanna hear is you complaining your back hurts because nobody told you not to slouch like a potato for hours.{w=0.75}{nw}"
            extend  " I-{w=0.2}I'm not having {i}that{/i} on my conscience."

        n  "Heh.{w=0.75}{nw}"
        extend  " In fact..."
        n  "You know what,{w=0.2} [player]?{w=0.75}{nw}"
        extend  " I hope you're sitting."
        n  "'Cause I'm gonna make sure you're using your computer the {i}right{/i} way,{w=0.5}{nw}"
        ...
        extend  " You got a minute?{w=0.75}{nw}"
        extend  " I gotta ask a question."
        n "..." # think
        n  "So...{w=1}{nw}"
        extend  " you've been visiting me for a while already,{w=0.2} huh?{w=0.75}{nw}"
        ...
        n  "But...{w=1}{nw}"
        extend  " I gotta ask."
        n  "Just how much of that exactly do you {i}actually{/i} remember,{w=0.5}{nw}"
        extend  " [player]?{w=0.75}{nw}"
        extend  " Huh?"
        n  "..."
        n  "Ehehe.{w=0.75}{nw}"
        extend  " Yep,{w=0.2} just as I thought.{w=0.75}{nw}"
        extend  " Total silence!"
        n  "Well [player],{w=0.2} have no fear."
        n  "'Cause it's time for a little refresher on how {i}not{/i} to ruin your spine from yours truly!"

    n  "Alright.{w=0.75}{nw}"
    extend  " So number one:{w=0.2}" 
    extend  " posture!"
    n  "...And no,{w=0.2} [player].{w=0.75}{nw}"
    extend  " I mean {i}actual{/i} posture.{w=0.75}{nw}"
    extend  " The sitting kind."
    n  "Now sit up straight,{w=0.2} and keep that back of yours against the chair,{w=0.2} [player].{w=0.75}{nw}"
    extend  " I mean it!"
    n  "Seriously -{w=0.5}{nw}"
    extend  " unless you feel like making friends with your nearest chiropractor,{w=0.5}{nw}"
    extend  " trust me when I say you {i}really{/i} don't wanna spend all your time slouching:{w=0.75}{nw}"
    extend  " or hunched up on your chair like some kind of weird computer gremlin."
    n  "If you're doing it right,{w=0.5}{nw}"
    extend  " then you should have your arms and thighs parallel to the ground,{w=0.5}{nw}"
    extend  " with your eyes roughly at the top of your screen.{w=0.75}{nw}"
    extend  " Easy peasy!"
    n  "Oh,{w=0.2} right -{w=0.5}{nw}"
    extend  " make sure your feet can still touch the floor,{w=0.2} though.{w=0.75}{nw}"
    extend  " Even {i}I{/i} can do that!"
    n  "'Next up,{w=0.2} number two:{w=0.75}{nw}"
    extend  " distance!"
    n  "It's pretty easy to forget,{w=0.5}{nw}"
    extend  " but if you wanna avoid sore eyes then you gotta make sure you're sitting a sensible distance from the screen too.{w=0.75}{nw}"
    extend  " Not right up close or a whole room away!"

    if Natsuki.isEnamored(higher=True):
        n  "I-{w=0.2}I know you just can't get enough of me,{w=0.2} [player].{w=0.75}{nw}"
        extend  " But really.{w=0.75}{nw}"
        extend  " Even I don't wanna see you practically pressing your face against the screen."

    elif Natsuki.isAffectionate(higher=True):
        n  "I-{w=0.2}I know I {i}am{/i} pretty awesome,{w=0.5}{nw}"
        extend  " but the last thing I wanna see is you pressing your face right up against the screen."

    else:
        n  "A-{w=0.2}and besides,{w=0.5}{nw}"
        extend  " the last thing I wanna see is your face all smushed up against the screen,{w=0.2} [player].{w=0.75}{nw}"
        extend  " I didn't ask for {i}that{/i}."

    n  "So...{w=1}{nw}"
    extend  " just make sure you're sitting about an arm's length away from the screen.{w=0.75}{nw}"
    extend  " That's all I'm saying!"
    n  "Don't forget to keep all your stuff in easy reach though -{w=0.5}{nw}"
    extend  " you can bet even Sayori wouldn't mess that up."
    n  "Now,{w=0.2} number three:{w=0.75}{nw}"
    extend  " breaks!"
    n  "I don't know about you [player],{w=0.5}{nw}"
    extend  " but personally?{w=0.75}{nw}"
    extend  " I can't stand being stuck in one place for hours at a time."
    n  "Seriously -{w=0.5}{nw}"
    extend  " I get all distracted and fidgety for some reason if I'm stuck in front of my laptop for too long...{w=1}{nw}"
    extend  " it's the worst!"
    n  "Plus on top of that,{w=0.5}{nw}"
    extend  " you're {i}really{/i} not doing your circulation a favor either."
    n  "...So get off your butt and do some stretches or something!{w=0.75}{nw}"
    extend  " Or even go get some water if you {i}really{/i} need an excuse to get moving."
    n  "Really,{w=0.2} it doesn't really matter exactly what you do -{w=0.5}{nw}"
    extend  " it's all about doing something to get you off your butt and back on your feet.{w=0.75}{nw}"
    extend  " Simple enough!"
    n  "..."
    n  "Well?{w=0.75}{nw}"
    extend  " Still following me,{w=0.2} [player]?"
    n  "You better be.{w=0.75}{nw}"
    extend  " This one's easily the most important,{w=0.2} so listen up!"
    n  "Now don't get me wrong,{w=0.2} [player] -{w=0.5}{nw}"
    extend  " I get that you're going to know your limits better than I do,{w=0.2} obviously."
    n  "But...{w=1}{nw}" 
    extend  " if you start feeling kinda weird or sick,{w=0.2} or your eyes start hurting or something while you're here?"
    n  "Just..."
    n  "..."
    n  "...Just don't be a total dummy about it.{w=0.75}{nw}"
    extend  " Alright?{w=0.75}{nw}"
    extend  " I'm being serious here."
    n  "If even being in front of the screen is starting to have some kind of impact..."
    n  "Call it quits on whatever you were doing and just come back to it later.{w=0.75}{nw}"
    extend  " Got it?"
    n  "Work or some dumb old assignment can wait if you're just gonna make yourself even worse trying to impress someone.{w=0.75}{nw}"
    extend  " Your professor isn't going to drop dead if some stinky report isn't done today."
    n  "Heh.{w=0.75}{nw}"
    extend  " And anyway,{w=0.2} think about it."
    n  "It isn't like you're going to pull some kind of miracle if you try and push through it all.{w=0.75}{nw}"
    extend  " All you're ending up doing is making yourself feel all crappy for longer.{w=0.75}{nw}"
    extend  " It's just pointless."
    
    if Natsuki.isEnamored(higher=True):
        n  "...And besides,{w=0.2} [player]."
        ...
        extend  " care about you,{w=0.2} [chosen_descriptor].{w=0.75}{nw}"
        extend  " You should really know that by now."
        n  "...Just like you should know you I'm {i}not{/i} gonna be impressed by some weird macho display of toughing it out."
        n  "Capiche?"
        n  "..."
    ...
    if Natsuki.isAffectionate(higher=True):
        ...
        n  "A-{w=0.2}and besides,{w=0.2} [player]."
        n  "It's not like I'm gonna get mad at you or anything dumb like that either.{w=0.75}{nw}"
        extend  " Even I'm not that much of a jerk."
        n  "..."

    n  "A-{w=0.2}anyway."
    n  "I've gone on way long enough already,{w=0.5}{nw}"
    extend  " so I'm just gonna say this,{w=0.2} [player]:"
    ...
    if Natsuki.isLove(higher=True):
        $ chosen_tease = jn_utils.getRandomTease()
        n  "Love you too,{w=0.2} [chosen_tease]!"

    else:
        $ chosen_descriptor = jn_utils.getRandomTease() if Natsuki.isEnamored(higher=True) else player
        n  "You're welcome,{w=0.2} [chosen_descriptor]!"
