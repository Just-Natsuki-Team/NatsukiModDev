...
label talk_favorite_subject:
    if Natsuki.isNormal(higher=True):
        ...
        n 1fsrwr "Measure my cake slices?!"
        ...
        n  "..."
        n  "...I guess I can at least tell you what I {i}don't{/i} like.{w=1}{nw}"
        extend  " Math."
        n  "...And no,{w=0.2} not because I'm just hopeless at it or anything stupid like that.{w=0.75}{nw}"
        extend  " Despite what you probably think."
        n  "It's just...{w=1}{nw}"
        extend  " it's way too complicated."
        extend  " It's ridiculous!"
        n  "I mean...{w=0.75}{nw}"
        extend  " how is {i}anyone{/i} meant to remember so much random nonsense?{w=0.75}{nw}"
        extend  " Especially around exams."
        n  "Because nothing makes me {i}love{/i} a subject more than having to no-life it{w=0.5}{nw}" 
        extend  " every{w=0.3} single{w=0.3} time{w=0.3}{nw}" 
        extend  " an exam shows up."
        n  "..."
        n  "I...{w=0.75}{nw}"
        extend  " get...{w=0.75}{nw}"
        extend  " that I could have just asked for help.{w=1}{nw}"
        extend  " From {i}my friends{/i} anyway."
        n  "But that's not the point."
        n  "Sometimes you just want to do stuff yourself.{w=0.75}{nw}"
        extend  " To not {i}need{/i} to rely on someone else constantly.{w=0.75}{nw}"
        extend  " And you know what?"
        n  "Why not?{w=0.75}{nw}"
        extend  " Besides..."
        n  "Not like {i}someone{/i}'s proving otherwise lately."
        n  "...Right,{w=0.75}{nw}" 
        extend  " {i}[player]{/i}?"
