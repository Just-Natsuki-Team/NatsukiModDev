...
label talk_favorite_subject:
    if Natsuki.isNormal(higher=True):
        n 1cslem "Not {i}math{/i},{w=0.5}{nw}"
        extend 1fsrfl " I'll tell you that much."
        n 1nsrsl "..."
        n 1cllslsbr "..."
        n 1cnmflsbr "...What?{w=0.75}{nw}"
        extend 1fllgs " I'm being serious!{w=0.75}{nw}"
        extend 1fbkwr " I just can't stand it!"
        n 1fcsgs "I mean,{w=0.2} I totally get all the normal stuff.{w=0.75}{nw}"
        extend 1flrfl " Addition,{w=0.2} subtraction,{w=0.2} fractions.{w=0.5}{nw}"
        extend 1fcsca " That's child's play.{w=0.75}{nw}"
        extend 1clrca " I don't really care about {i}that{/i}."
        n 1fcsfl "It's just..."
        n 1fsrun "..."
        n 1fcswr "It's when you get all those {i}stupid{/i} rules you have to remember!{w=0.75}{nw}"
        extend 1fbkwr " There's just way too many of them!"
        n 1fllwr "Like...{w=1}{nw}"
        extend 1fcsgs " how am {i}I{/i} meant to remember three different flavours of mumbo-jumbo for calculating some random part of a triangle?{w=0.75}{nw}"
        extend 1flran " What am I even gonna {i}do{/i} with that?"
        n 1fsrwr "Measure out the perfect pizza slice?!"
        n 1fcsan "And don't even get me {i}started{/i} on calculus...{w=1}{nw}"
        extend 1fslsl " or {i}algebra{/i}."
        n 1fsqem "And I {i}really{/i} hated having to spend my nights cramming the crap out of it before{w=0.5}{nw}"
        extend 1fcsan " every{w=0.3} single{w=0.3}{nw}" 
        extend 1fsran " exam."
        n 1fcsan "It's..."
        n 1csrsl "..."
        n 1ccsflsbr "...Just not how my brain works.{w=0.75}{nw}"
        extend 1cslbolsbr " At all."
        n 1fcsgslsbr "B-{w=0.2}but that's not to say nobody would help or anything like that!{w=0.75}{nw}"
        extend 1fllflsbr " Monika always kept trying to offer when she had the time.{w=0.75}{nw}"
        extend 1cllbo " But..."
        n 1kllbo "..."

        if Natsuki.isAffectionate(higher=True):
            n 1csrbolsbr "...It's just embarassing.{w=0.75}{nw}"
            extend 1knmbolsbr " You know?{w=0.75}{nw}"
            extend 1fcsfl " And as much as I can't stand the subject..."
            ...
            n 1ccsfllsbr "A-{w=0.2}anyway!{w=0.75}{nw}"
            extend 1fcspo " We're getting off track here."
            n 1tllpu "Favorite subject,{w=0.5}{nw}" 
            extend 1tnmss " huh?"
            n 1csqsm "Ehehe.{w=0.75}{nw}"
            extend 1fsqbg " You really need to ask?{w=0.75}{nw}"
            extend 1fcsbg " You hadn't {i}already{/i} guessed,{w=0.2} [player]?"

        else:
            n 1ccsfllsbl "...Forget it.{w=0.75}{nw}"
            extend 1csrcalsbl " It doesn't matter."
            n 1fcsajlsbl "A-{w=0.2}and besides.{w=0.75}{nw}"
            extend 1fcspo " This was already getting way off track."
            n 1unmaj "Favorite subject,{w=0.5}{nw}"
            extend 1tnmbo " huh?"
            n 1ccsss "Heh.{w=1}{nw}"
            extend 1fcsbg " As if the answer hadn't already practically slapped you in the face."
        
        n 1uchgn "...Literature,{w=0.75}{nw}"
        extend 1fchgn " duh!"
        n 1fsqsm "You didn't seriously think I just pulled my poetry out of thin air or something,{w=0.5}{nw}"
        extend 1fsqbg " did you?{w=0.75}{nw}"
        extend 1fchgn " Nope!"
        n 1fllss "I mean,{w=0.75}{nw}"
        extend 1fcsss " I {i}am{/i} a natural..."
        n 1fcstr "...But you're only kidding yourself if you think you can get away without putting in actual effort too."
        n 1tllaj "Really though -{w=0.5}{nw}"
        extend 1tnmfl " how could anyone dislike something with so much variety?{w=0.75}{nw}"
        extend 1fspgs " Literature is {b}awesome{/b}!"
        n 1fcsbg "Epic stories...{w=0.75}{nw}"
        extend 1unmfl " life-changing experiences...{w=0.75}{nw}"
        extend 1clrss " anything and everything you could {i}possibly{/i} imagine..."
        n 1tsqfl "...And all I gotta do is turn the pages?{w=0.75}{nw}"
        extend 1fchgn " Sign me up!"
        n 1flrss "I've probably called you it a bunch before already,{w=0.2} [player]."
        n 1fsqsm "...But only a {i}real{/i} dummy would check out on an offer that good."
        extend 1fcssm " Ehehe."
        n 1ulraj "Well,{w=0.2} anyway.{w=0.75}{nw}"
        extend 1tlrbo " I think I've gone on about that long enough.{w=0.75}{nw}"
        extend 1tsqfl " Who wants to spend all day talking about courses and topics of all things?"

        if Natsuki.isLove(higher=True):
            n 1ccsbgl "Besides,{w=0.2} [player]."
            n 1fcsbgl "Don't you recognize a far more interesting subject..."
            n 1fsqssl "...When it's already sat right in front of you?{w=0.75}{nw}"
            extend 1fsldvlsbr " Ehehe."
            $ chosen_tease = jn_utils.getRandomTease()
            n 1fchbgfsbr "L-{w=0.2}love you too,{w=0.2} [chosen_tease]!"

        elif Natsuki.isEnamored(higher=True):
            n 1fcsssl "Besides..."
            n 1fcsbglsbr "D-{w=0.2}don't you have something {i}way{/i} more interesting in front of you that you should be learning from instead?{w=0.75}{nw}"
            extend 1fslsslsbr " Ehehe..."

        else:
            n 1csqbg "In fact..."
            n 1fcsss "I guess you could say it's about time we...{w=0.75}{nw}"
            extend 1fsqbg " {i}changed the subject{/i}{w=0.75}{nw}"
            extend 1fchgn ",{w=0.2} right?"
            n 1nchgn "Ehehe."

    elif Natsuki.isDistressed(higher=True):
        n 1fcssl "..."
        n 1fslsl "...I guess I can at least tell you what I {i}don't{/i} like.{w=1}{nw}"
        extend 1fcsfl " Math."
        n 1fsqfl "...And no,{w=0.2} not because I'm just hopeless at it or anything stupid like that.{w=0.75}{nw}"
        extend 1fsrem " Despite what you probably think."
        n 1fcsem "It's just...{w=1}{nw}"
        extend 1fcsfl " it's way too complicated."
        extend 1fllan " It's ridiculous!"
        n 1fcsan "How is {i}anyone{/i} meant to remember so much random nonsense?{w=0.75}{nw}"
        extend 1flrem " Especially around exams."
        n 1fupfl "Because nothing makes me {i}love{/i} a subject more than having to no-life it{w=0.5}{nw}" 
        extend 1fcsan " every{w=0.3} single{w=0.3} time{w=0.3}{nw}" 
        extend 1fslan " an exam shows up."
        n 1fcssl "..."
        n 1fcsfl "I...{w=0.75}{nw}"
        extend 1fsran " get...{w=0.75}{nw}"
        extend 1fsrsr " that I could have just asked for help.{w=1}{nw}"
        extend 1fsqem " From my {w=0.2}{i}friends{/i}{w=0.2} anyway."
        n 1clrsl "But that's not the point."
        n 1csrsl "Sometimes you just want to do stuff yourself.{w=0.75}{nw}"
        extend 1fsrfr " To not {i}need{/i} to rely on someone else constantly.{w=0.75}{nw}"
        extend 1fnmfl " And you know what?"
        n 1fllfl "Why not?{w=0.75}{nw}"
        extend 1fslan " Besides..."
        n 1fslem "Not like there's always gonna be {i}someone{/i} you can rely on."
        n 1fllem "...Right,{w=0.75}{nw}" 
        extend 1fsqem " {i}[player]{/i}?"

    else:
        n 1fsqan "...Really?{w=0.75}{nw}"
        extend 1fsqfl " My favorite subject?"
        n 1fnman "...And why exactly should I tell {i}you{/i}?"
        n 1fsqfutsb "So you can start lecturing me about it?"
        n 1fcssltsa "Heh.{w=1}{nw}"
        extend 1fcsemtsa " Yeah.{w=1}{nw}"
        extend 1fsqantsb " {i}No thanks{/i}."
        n 1fsqupltsb "Now {i}get out of my face{/i}."
