...
label talk_how_to_interview_properly:
    ...
    n  "Hey,{w=0.5}{nw}"
    extend " [player]."
    n  "It's kinda out of the blue,{w=0.5}{nw}"
    extend " but I was curious."
    n  "When was the last time you interviewed for something?"
    n  "Or now that I think about it...{w=1}{nw}"
    extend " have you interviewed for {i}anything{/i} before?{w=1}{nw}"
    extend " Like,{w=0.1} at all?"
    n  "Because if there's one thing I've heard...{w=1}{nw}"
    extend " it's how anxious everyone seems to get over interviewing!"
    n  "I'm being serious!{w=0.5}{nw}"
    extend " People just get so worked up over it all.{w=1}{nw}"
    extend " Like it's rocket science or something."
    n  "I mean...{w=1}{nw}"
    extend " I've never had to interview for anything super important myself."
    n  "We had some practice interviews at school,{w=0.1} obviously.{w=1}{nw}"
    extend " I was too busy with studies to try at getting a part-time job or anything."
    n  "...But who says that doesn't mean I can teach you a thing or two?{w=0.75}{nw}"
    extend " Ehehe."
    n  "You should know what time it is by now..."
    n  "...So listen up,{w=0.1} [player]!"
    ...
    n  "Research,{w=0.5}{nw}"
    extend " duh!"
    n  "If there's one thing you gotta know before going to interview for something,{w=0.5}{nw}"
    extend " it's what you're actually interviewing {i}for{/i}!"
    n  "You wouldn't skimp out on revising before a big test,{w=1}{nw}"
    extend " and this really isn't much different when you think about it."
    n  "Interviewing for some big-shot company?{w=1}{nw}"
    extend " Check them out online and take notes!"
    n  "Obviously you'll want to read up on what they do and where they actually are,{w=1}{nw}"
    extend " but don't underestimate the power of trivia!"
    n  "Even just knowing random stuff like when the company was founded,{w=1}{nw}"
    extend " or what awards they won recently {w=0.1}-{w=0.5}{nw}"
    ...
    n  "Next up...{w=0.5}{nw}"
    extend " Revision!"
    n  "It doesn't matter if you're trying to get a job,{w=1}{nw}"
    extend " or land a new position on some sort of council."
    n  "Whatever it is...{w=0.5}{nw}"
    extend " you gotta be able to prove you know what you're talking about!"
    n  "Some kind of programming job?{w=0.5}{nw}"
    extend " Refresh yourself on all your weird terminology and techniques!{w=1}{nw}"
    extend " Joining the history club?{w=1}{nw}"
    extend " Read up on some common trivia questions!"
    n  "And trust me,{w=0.75}{nw}"
    extend " the {i}last{/i} thing you wanna do is embarrass yourself over simple stuff you should {i}really{/i} know..."
    n  "...Or something you forgot you mentioned on your application."
    n  "So...{w=0.5}{nw}"
    extend " study up,{w=0.1} 'kay?"

    n  "Now..."
    n  "Keeping up so far,{w=0.1} [player]?"
    n  "You better be...{w=1}{nw}"
    extend " 'cause we're almost done here!"
    n  "Next on the list -{w=0.5}{nw}"
    extend " and probably the most important of all..."
    n  "Presentation!"
    n  "You can have the best credentials in the world,{w=1}{nw}"
    extend " but that isn't gonna help much if you're mumbling everything {w=0.1}-{w=0.5}{nw}"
    extend " or if you just look ridiculous."
    n  "So!"
    n  "Make sure you dress properly for whatever it is.{w=1}{nw}"
    extend " If there's a dress code,{w=0.1} {i}follow it{/i}."
    n  "...And {i}don't{/i} flake out on your clothes.{w=1}{nw}"
    extend " Iron them if they're all creased,{w=0.1} that kind of thing."
    n  "But most of all,{w=0.1} [player]?"
    n  "{i}Never{/i} forget the basics."
    n  "Be punctual,{w=0.1} be polite.{w=0.2} Remember {w=0.1}-{w=0.5}{nw}"
    extend " people want someone they can {i}like{/i},{w=0.75}{nw}"
    extend " not just get the job done!"
    
    n  "...Oh,{w=1}{nw}"
    extend " and [player]?"
    n  "Just...{w=1}{nw}"
    extend " be honest too,{w=0.1} alright?"
    n  "It isn't a fault to admit when you don't know something."
    n  "And when you actually stop to think about it from their perspective,{w=1}{nw}"
    extend " if someone is prepared to just lie to your face at an interview..."
    n  "...Then what {i}else{/i} are they gonna lie about?"
    n  "Food for thought, huh?"

    n "..."
    n  "...Wow,"
    extend " I gotta learn when to stop rambling on!{w=0.2} Jeez!"
    n  "That was almost like an interview speech itself..."

    if Natsuki.isEnamored():
        n  "Or...{w=1}{nw}"
        extend " I guess more like an induction,{w=0.1} really."
        n  "You already got the job with me,{w=0.1} a-{w=0.3}after all."

        if Natsuki.isLove():
            n  "Ehehe.{w=1}{nw}"
            extend " Love you,{w=0.1} [player]~!"
        ...
        n  "Or...{w=1}{nw}"
        extend " since we're both stuck here?"
        n  "...More like an induction,{w=0.1} actually."
        n  " Ehehe."
