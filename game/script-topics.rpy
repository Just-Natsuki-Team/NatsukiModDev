...
label talk_can_you_draw:
    ...
    if Natsuki.isNormal(higher=True):
        if already_discussed_topic:
            ...
            n 2fslpo "I'm still kinda peeved about how my school handled art classes.{w=0.75}{nw}"
            extend 2nnmaj " Like yeah,{w=0.2} we had them...{w=1}{nw}"
        ...
        extend 4ctlsr " And of course there's tons of styles out there to learn.{w=0.75}{nw}"
        extend 1fcspu " I get it."
        n 2flrgs "But come{w=0.5}{nw}" 
        ...
        n 2fsqem "...If we barely even had the time to cover any of the basics?{w=0.75}{nw}"
        extend 2fcsup " Let alone try any of the stuff we studied out!"
        n 2cslem "A-{w=0.2}and besides!{w=0.75}{nw}"
        extend 2cslfl " All those artists already had their turn.{w=0.75}{nw}"
        extend 2ksqbol " I wanna create cool things too!"
        n 1ccsemesisbl "Ugh..."
        ...
        extend 2kslpu " Like at home or whatever.{w=0.75}{nw}"
        extend 2fnmbo " Or maybe even the art club!"
        n 2nslss "Heh."
        n 1cslsmsbr "No prizes for guessing why neither of those were gonna work out."
        n 1kslfs "..."
        n 2cnmaj "Putting all that aside though.{w=0.75}{nw}"
        extend 2tnmss " Honestly?"
        n 2csrpul "It's not that I wouldn't want to be able to draw cool stuff.{w=0.75}{nw}"
        extend 2cnmaj " Are you kidding?{w=0.75}{nw}"
        extend 1fchgn " That'd be awesome!"

        if Natsuki.isAffectionate(higher=True):
            n 2csrsssbl "It's just that..."
            n 2ksrfssbl "..."
            n 2kcssl "It's...{w=1}{nw}" 
            extend 1cslpu " intimidating.{w=1}{nw}"
            extend 2fslpu " Trying to teach yourself something like that."
            ...
            if Natsuki.isEnamored(higher=True):
                n 2clrssl "People always say it's just down to practice,{w=0.75}{nw}"
                extend 2ksrfslsbl " or to look up some random course online or whatever.{w=0.75}{nw}"
                extend 1fnmflsbl " A-{w=0.2}and I'm not saying they're wrong!"
                n 4nslaj "Everyone has their own approach,{w=0.2} obviously."
                n 2cnmbol "...I guess I just never found mine.{w=0.75}{nw}"
                extend 2csrssl " Not {i}yet{/i},{w=0.2} anyway."
                n 2nlrpu "Though...{w=1}{nw}"
                extend 4tnmss " what about you,{w=0.2} [player]?"
            
            else:
                n 2clrsr "..."
                n 2csrpu "I don't know."
                n 2ncsbo "..."
                n 1fchsslsbl "A-{w=0.2}anyway!{w=0.75}{nw}"
                extend 4fslfslsbl " I'm kinda bored of listening to myself now.{w=0.75}{nw}"
                extend 4tnmss " So what about you,{w=0.2} [player]?"

        else: 
            n 2clrca "But...{w=1}{nw}"
            extend 2ksrpu " that's..."
            n 1fcsbo "..."
            n 1fslsslsbr "N-{w=0.2}nevermind."
            extend 1cllsl " I guess it doesn't even matter now anyway."
            n 4cchsmsbl "Besides.{w=0.2} I think I'm about done listening to myself talk for now."
            n 4nlraj "So...{w=0.75}{nw}"
            extend 4tnmss " what about you,{w=0.2} [player]?"

        if jn_activity.ACTIVITY_MANAGER.hasPlayerDoneActivity(jn_activity.JNActivities.artwork):
            n 4fwdaw "...Wait!{w=0.75}{nw}"
            extend 1fchgn " Don't tell me!{w=0.75}{nw}"
            extend 3fcsbs " I've got a pretty good hunch for this kind of thing,{w=0.5}{nw}"
            extend 3fsqss " if I say so myself."
            n 3fchsm "Ehehe."
            n 3fcsaj "Yep!"
            n 1nsqcs "You've got 'artist' written all over you."
            n 4fsgct "All the more reason to be grateful you've always got such a great subject on hand,{w=0.5}{nw}" 
            ...
            menu:
                ...
                "Yes,{w=0.2} I draw often!":
                    ...
                    extend 3tsqct " Fancy yourself an artist,{w=0.2} huh?"
                    n 3fchsm "Ehehe."
                    n 3nllss "Well,{w=0.5}{nw}"
                    extend 3fsqcs " you better consider yourself lucky,{w=0.2} [player].{w=0.75}{nw}"
                    extend 3fchgn " Not every day such an amazing subject just plonks itself down right in front of you!"

                "No,{w=0.2} I don't usually draw.":
                    n 3tllaj "Really?{w=0.75}{nw}"
                    extend 3kslpu " Aww..."
                    n 3knmfl "I'll admit,{w=0.2} I am kinda disappointed.{w=1}{nw}"
                    extend 4clrss " I get it though.{w=0.75}{nw}"
                    extend 2ksrca " Not like {i}I'm{/i} one to judge for that anyway."

                "I don't draw anymore.":
                    n 3kslpu "Aww..."
                    n 3tslbo "..."
                    n 3tnmfl "...Wait.{w=0.2} 'Anymore'?{w=0.75}{nw}"
                    extend 3csqem " What do you mean,{w=0.2} {i}'anymore'{/i}?{w=0.5}{nw}"
                    extend 3fcsbo " You gotta get back into it,{w=0.2} [player]!"
                    n 3tlrss "Why not?{w=0.75}{nw}"
                    extend 3fnmss " {i}Especially{/i} when you've been blessed with {i}such{/i} a good subject to get started!{w=0.75}{nw}"
                    extend 3fchsm " Ehehe."

        n 1nllss "Well anyway.{w=0.75}{nw}"
        extend 2tnmss " You know what?{w=0.75}{nw}"
        extend 2flrbg " Who's to say I couldn't learn to draw at some point?"
        n 2utraj "There's gotta be some art supplies I can steal from somewhere around here."
        n 2nsrpo "Not like the art club's gonna need 'em.{w=0.75}{nw}"
        extend 1tsqct " And who knows,{w=0.2} [player]?"
        n 3fcscsesm "I bet there's a Picasso in me yet!"
        n 3nnmss "I just gotta{w=0.5}{nw}"
        extend 3fsqss " {i}ease-l{/i}{w=0.5}{nw}"
        extend 3fchgn " into it first!{w=0.75}{nw}"
        extend 3fchgn " Ehehe."

        if Natsuki.isLove(higher=True):
            n 3kslssl "Love you too,{w=0.2} [player]~!"

    elif Natsuki.isDistressed(higher=True):
        if already_discussed_topic:
            n 2fsqsl "..."
            n 4fcsan "No,{w=0.2} [player].{w=1}{nw}"
            extend 4fsqan " Like I {i}already{/i} told you.{w=0.75}{nw}"
            extend 4fslem " I can't."
            n 1fllsf "What did you expect?{w=0.75}{nw}"
            extend 2fsqup " I already said how my art classes sucked{w=0.75}{nw}"
            extend 2fcswr " if you were even paying attention."

        else:
            n 2fsqan "No,{w=0.2} [player].{w=0.75}{nw}"
        ...
        n 1fnman "It was just a complete disappointment."
        n 1fcsbo "..."
        n 4fnmfl "Look.{w=0.2} I'm not dumb.{w=1}{nw}"
        extend 2fsrem " I get that you gotta learn about why people do art,{w=0.75}{nw}"
        extend 2nslsf " or all the different styles and how they work."
        n 2flrgs "But come{w=0.5}{nw}"
        ...
        extend 2ccsup " Not all of us have the time for an art club."
        ...
        extend 2cllsf " Not like spare time is a problem anymore."
        n 2tslpu "But honestly?"
        n 2fslfr "{i}Something{/i} tells me I'm not gonna be in the mood for that a whole lot."
        n 2ttlfl "I wonder why..."
        extend 2fsqem " {i}[player]{/i}."
        ...
        if already_discussed_topic:
            ...
            n 2fnmup "The only thing I'm drawing right now is an end to this {i}stupid{/i} conversation."
            n 4fnman "Jerk."

        else:
            n 2fcssstsa "...Heh.{w=0.75}{nw}"
            extend 2fslsltsb " Can I draw?{w=1}{nw}"
            extend 2fsqfltsb " Really?"
            n 2fcssf "..."
            n 2fslup "...Actually.{w=0.75}{nw}"
            extend 2fsqfu " You know what?"
            n 2fnmwrtsc "Yeah.{w=0.2} I am.{w=0.75}{nw}"
            extend 2fsqfutse " At drawing conclusions."
            n 2fcsemltse "...And I've got plenty drawn about the likes of {b}you{/b}."
