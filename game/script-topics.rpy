...
label talk_thoughts_on_vegetarianism:
    ...
    if Natsuki.isNormal(higher=True):
        if already_discussed_vegetarianism:
            ...
            extend  " Vegetarianism?"
            n  "That's...{w=0.75}{nw}"
            extend  " definitely not a question I was expecting,{w=0.75}{nw}"
            extend  " I gotta say."
            n  "But...{w=0.75}{nw}" 
            extend  " Why do you ask,{w=0.2} [player]?"
            n  "Looking to turn over a new leaf,{w=0.2} are we?{w=0.75}{nw}"
            extend  " Ehehe."

        n  "Well,{w=0.2} anyway.{w=0.75}{nw}"
        extend  " In all seriousness?"
        n  "I've never really thought about it that much,{w=0.75}{nw}"
        extend  " to be honest."
        n  "We had vegetarian and vegan options at school,{w=0.75}{nw}"
        extend  " but I never really felt a particular drive to try them out or anything."
        extend " Not like I can really try them {i}now{/i},{w=0.2} either."
        n  "T-{w=0.2}that's not to say they looked {i}bad{/i} or anything!"
        n  "At least not to me."
        n  "Heck,{w=0.75}{nw}"
        extend  " Monika shoveled that stuff down like there was no tomorrow!"
        n  "Although...{w=1}{nw}"
        extend  " I do think it's super admirable how people can put meat aside for so many reasons!{w=1}{nw}"
        extend  " Seriously!"
        n  "I mean,{w=0.75}{nw}"
        extend  " it's one thing to skip out on eating a particular type of meat.{w=0.75}{nw}"
        extend  " Anyone can do that."
        n  "But dropping {i}everything{/i},{w=0.75}{nw}" 
        extend  " or even refusing {i}any{/i} animal products whatsoever?"
        n  "Now {i}that{/i} takes some guts!{w=0.75}{nw}"
        extend  "...Heh.{w=0.75}{nw}"
        extend  " Pun not intended."

        n  "Of course,{w=0.2} maybe some people just don't like the taste.{w=0.75}{nw}"
        extend  " I totally get that.{w=1}{nw}"
        extend  " But there's a whole bunch of reasons people do it -{w=0.5}{nw}"
        extend  " and they're all just as valid!"
        n  "Environmental concerns,{w=0.5}{nw}"
        ...
        n  "Oh -{w=0.5}{nw}" 
        extend  " don't worry,{w=0.2} [player].{w=0.75}{nw}"
        extend  " I'm not gonna get all preachy about it or anything like that."
        n  "It {i}is{/i} just a choice like anything else,{w=0.75}{nw}"
        extend  " after all.{w=0.75}{nw}"
        extend  " And a personal one at that."

        n  "...Not like that's gonna stop me suggesting it though!"
        n  "Even if {i}I{/i} can't try it out myself right now..."
        n  "There's no excuse for you!{w=1}{nw}"
        extend  " Sorry,{w=0.2} [player]."
        n  "You aren't wriggling out of this one!{w=0.75}{nw}"
        extend  " Ehehe."
        n  "I don't know if you're practicing an animal-free diet already,{w=0.75}{nw}"
        extend  " but if you aren't..."
        n  "Why not give it a shot,{w=0.2} [player]?"
        n  "You don't need to go all out or anything -{w=0.5}{nw}"
        extend  " you could swap out one meal a day and just see how you feel!"
        n  "Plus with all the recipes out there,{w=0.75}{nw}"
        extend  " the hard work is practically already done for you!{w=0.75}{nw}"
        extend  " Easy peasy!"
        n  "And anyway,{w=0.75}{nw}"
        extend  " even if it's only for a day or two,{w=0.2} and it doesn't work out:{w=0.5}{nw}"
        extend  " at least you can say you {i}tried{/i} it,{w=0.2} right?"

        n  "Well,{w=0.2} at any rate -{w=0.5}{nw}"
        extend  " I've gone on long enough,{w=0.75}{nw}"
        extend  " so I think that's about all I have to say on that topic."
        n  "Besides,{w=0.2} [player]..."
        n  "Wouldn't want to turn you into a {i}vegetable{/i} now,{w=0.75}{nw}"
        extend  " would we?"
        n  "Ehehe."
