...
label talk_did_you_have_pets:
    ...
    if persistent._jn_player_pet == "birds":
        ...
        extend 1tnmss " Birds,{w=0.2} huh?{w=0.75}{nw}"
        extend 1fchsm " Neat!"
        n 1flrss "I bet you got into a total{w=0.5}{nw}"
        extend 1fsqbg " {i}flap{/i}{w=0.75}{nw}"
        extend 1usqbg " when you saw yours for the first time,{w=0.2} huh?"

        if Natsuki.isLove(higher=True):
            n 1fcssmeme "Ehehe."
            extend 1fwlbgl " Love you,{w=0.2} [player]~!"

        else:
            n 1fchsmeme "Ehehe."

    elif persistent._jn_player_pet == "cats":
        n 1unmbg "Kitties?{w=0.75}{nw}"
        extend 1uchgn " {i}Now{/i} we're talking,{w=0.2} [player]!"

        if Natsuki.isAffectionate(higher=True):
            n 1clraj "Well..."
            n 1ccsbgsbl "Unless you just said that hoping {i}I{/i} like them too,{w=0.2} anyway."

        else:
            n 1nllaj "Though...{w=1}{nw}" 
            extend 1fllss " I've gotta ask."
            n 1fcsbg "...Just how many videos have you made {i}yours{/i} the star of,{w=0.5}{nw}" 
            extend 1fsqbg " [player]?"

        n 1usqsm "..."
        n 1fcssm "Ehehe.{w=0.75}{nw}"
        extend 1fcsbg " What's wrong,{w=0.2} [player]?"
        extend 1fchgn " Did I call you out?"
        n 1ulraj "Well,{w=0.2} anyway.{w=0.75}{nw}"
        extend 1fcsaj " You better be taking care of yours,{w=0.2} [player]!"
        n 1fcspo "...Or I really will give you {i}paws{/i} for thought."

    elif persistent._jn_player_pet == "chameleons":
        n 1tnmfl "Chameleons?{w=0.75}{nw}"
        extend 1unmbo " Like with the swivelly eyes and all that?{w=0.75}{nw}"
        extend 1clrbo " Huh."
        n 1clrfl "I...{w=1}{nw}" 
        extend 1tnmbo " didn't even know people kept those,{w=0.2} honestly.{w=0.75}{nw}"
        extend 1tllss " Neat."
        n 1fcssm "...Heh."
        n 1flrss "Well...{w=1}{nw}"
        extend 1fcsbg " {i}color{/i} me surprised!"
        extend 1fchsm " Ehehe."

        if Natsuki.isEnamored(higher=True):
            n 1nchgn "No regrets,{w=0.2} [player]!"

    elif persistent._jn_player_pet == "dogs":
        n 1unmaj "Dogs,{w=0.2} huh?{w=0.75}{nw}"
        extend 1fchsm " Cool!"
        n 1csrss "...Not that I can see myself ever owning one,{w=0.2} though.{w=0.75}{nw}"
        extend 1csrpo " With the walks every single day and all that."

        if get_topic("talk_skateboarding").shown_count > 0:
            n 1fcsaj "Nope!{w=0.75}{nw}"
            extend 1fcsbg " The skateboard is more than enough for me!"

        else:
            n 1fcsaj "Nope!{w=0.75}{nw}"
            extend 1fcsgs " I'll take my walks when {i}I{/i} want them,{w=0.5}{nw}"
            extend 1fcspo " thank you very much."
            n 1fsqsm "Ehehe."

        n 1ullaj "Well,{w=0.2} anyway -{w=0.5}{nw}"
        extend 1ccstr " better make sure you're taking care of yours,{w=0.2} [player]..."
        n 1csqpo "Or I really {i}will{/i} have a bone to pick with you."

    elif persistent._jn_player_pet == "ferrets":
        n 1clrpu "You know...{w=1}{nw}"
        extend 1cnmbo " I've always wondered about ferrets.{w=0.75}{nw}"
        extend 1tslpu " Is it more like having a cat or a dog?"
        n 1cslbo "Huh."
        n 1ullaj "Well anyway...{w=1}{nw}" 
        extend 1fcstr " whatever they're like.{w=0.75}{nw}"
        extend 1fcssmesm " You better be taking care of the little guy!"

        if Natsuki.isLove(higher=True):
            n 1flrssl "At least as much as you take care of me,{w=0.2} anyway."
            n 1tlrajl "Well...{w=1}{nw}"
            extend 1fsqssl " {i}almost{/i}.{w=0.75}{nw}"
            extend 1fchsml " Ehehe."

    elif persistent._jn_player_pet == "fish":
        n 1tllpu "Fish,{w=0.5}{nw}"
        extend 1tnmbo " huh?"
        n 1tlrbo "..."
        n 1tlrss "Well...{w=1}{nw}"
        extend 1csrss " I wouldn't call them {i}super{/i} affectionate or anything...{w=1}{nw}"
        extend 7ulrca " but I suppose I can see the reasons to keep them around.{w=0.75}{nw}"
        extend 7unmbg " Like for stress!"
        n 1fcsss "Heh."
        n 1fcssm "I bet you feel like you could lose yourself in that tank,{w=0.5}{nw}" 
        extend 1fsqsm " huh?{w=0.75}{nw}"
        extend 1fchsm " Ehehe."

    elif persistent._jn_player_pet == "frogs":
        n 1unmaj "Ooh!{w=0.2} Froggies!{w=0.75}{nw}"
        extend 1fchbg " Cute!"
        n 1fllss "I seriously can't get enough of their faces.{w=0.75}{nw}"
        extend 1uchgn " They always look so {i}confused{/i}!{w=0.75}{nw}"
        extend 1nchgn " Ahaha."
        n 1ulrss "Well,{w=0.2} anyway...{w=1}{nw}"
        extend 1tnmss " I guess I shouldn't keep you waiting,{w=0.2} [player].{w=0.75}{nw}"
        extend 1fsqss " You know."
        n 1fcsbg "Just in case you gotta {i}hop to it{/i} and take care of yours!"

        if Natsuki.isLove(higher=True):
            n 1fchsmleme "Ehehe.{w=0.75}{nw}"
            extend 1fchbll " Love you,{w=0.2} [player]~!"
        ...
        extend 1fchbg " Yeah!{w=0.75}{nw}"
        extend 1fchsm " I've heard about those!"
        n 1flrss "Man...{w=1}{nw}"
        extend 1fchgn " I just can't get over how goofy they look!{w=0.75}{nw}"
        extend 1cslbosbr " Even if you do have to feed them bugs."
        n 1ccstr "Just a warning though,{w=0.2} [player]..."
        n 1csqpo "I better not hear about any tails falling off on your watch."

    elif persistent._jn_player_pet == "gerbils":
        n 1ulraj "Gerbils,{w=0.5}{nw}"
        extend 1tnmbo " huh?"
        n 1fsqaj "...And why's that,{w=0.2} [player]?{w=0.75}{nw}"
        extend 1fnmfl " Hamsters not good enough for you?{w=0.75}{nw}"
        extend 1fnmaj " Is that it?"
        n 1fsqpo "..."
        n 1fcssm "Ehehe.{w=0.75}{nw}"
        extend 1ulrss " Nah,{w=0.2} I guess I can see it.{w=0.75}{nw}"
        extend 1tnmbo " You keep gerbils together,{w=0.2} right?{w=0.75}{nw}"
        extend 1fchsm " Cute!"
        n 1fcsaj "You better be taking care of yours though,{w=0.2} [player]..."
        n 1fsqss "...Or you really {i}will{/i} need them to dig you out of it.{w=0.75}{nw}"
        extend 1fcssm " Ehehe."
        ...
        n 1uwdaj "Ooh!{w=0.75}{nw}"
        extend 1uchbg " Yeah!{w=0.2} I've seen those!"
        n 1clrbgsbl "I don't exactly know {i}tons{/i} about them,{w=0.5}{nw}" 
        extend 1fchgn " but I love the little sounds they make!{w=0.75}"
        extend 1fchbg " Especially when you have a bunch of them together!"
        n 1fcssm "I bet yours are a total bunch of chatterboxes too,{w=0.2} huh?{w=0.75}{nw}"
        extend 1fchsm " Ehehe."

    elif persistent._jn_player_pet == "hamsters":
        n 1fcsaj "W-{w=0.2}wait.{w=0.75}{nw}"
        extend 1unmfl " Hammies?{w=0.75}{nw}"
        extend 1fnmfl " For real?"
        n 1fcsun "..."
        n 1fcsem "Are...{w=1}{nw}"
        extend 1fcsaj " you...{w=1}{nw}"
        extend 1fspgs " freaking KIDDNG ME?!{w=0.75}{nw}"
        extend 1uchbs " I {b}love{/b} hammies!"
        n 1fcsbgl "Those stubby little tails,{w=0.5}{nw}"
        extend 1flrbgl " those tiny paws...{w=1}{nw}"
        extend 1fchgnl " those chubby cheeks?"
        n 1fcssml "Ehehe.{w=0.75}{nw}"
        extend 1fcsbs " Yep!{w=0.75}{nw}"
        extend 1fllbg " When it comes to a {i}professional{/i} display of cuteness,{w=0.5}{nw}"
        extend 1fcsbg " I'm afraid I've gotta say it."
        n 1nchgn "You just can't beat hamsters!"
        n 1nllsssbr "Though...{w=1}{nw}"
        extend 1nslss " that being said."
        n 1csqtr "You better be taking {i}real{/i} good care of yours,{w=0.2} [player]..."

        if Natsuki.isAffectionate(higher=True):
            n 1nsrpo "...Or you really will find out how much I ham it up.{w=0.75}{nw}"
            extend 1fsqsm " Ehehe."
            
            if Natsuki.isLove(higher=True):
                n 1fchsmleaf "Love you too,{w=0.2} [player]~!"

        else:
            n 1fcspo "...Or you really {i}will{/i} find out how much I ham it up."

    elif persistent._jn_player_pet == "horses":
        n 1unmemesu "H-{w=0.2}huh?{w=0.75}{nw}"
        extend 1unmgssbl " Horses?{w=0.75}{nw}"
        extend 1tnmflsbl " Seriously?!"
        n 1clremsbr "Jeez...{w=1}{nw}"
        extend 1csqposbr " you aren't just messing with me,{w=0.2} are you?{w=0.75}{nw}"
        extend 1cllemsbr " How do you even {i}afford{/i} something like that,{w=0.2} [player]?"
        
        if get_topic("talk_careful_spending").shown_count > 0:
            n 1tllsl "..."
            n 1tllpu "...Huh.{w=0.75}"
            extend 1cslsssbr " Maybe that whole conversation about being careful with your money {i}was{/i} needed after all."

        else:
            n 1csrsssbr "...Even if it {i}is{/i} pretty cool."
        
        n 1ulraj "Well,{w=0.2} anyway.{w=0.75}{nw}"

        if Natsuki.isAffectionate(higher=True):
            extend 1fcsbg " I hope you're all saddled up,{w=0.2} [player]..."
            n 1nchgnelg "'Cause there's no way you're wiggling out of some free riding lessons!"

        else:
            extend 1nsqpo " You better hope I don't catch you riding without a helmet,{w=0.2} [player]."
            n 1ccspoesi "'Cause you getting saddled up with a bunch of dumb hospital bills is the last thing I wanna hear about."

    elif persistent._jn_player_pet == "insects":
        n 1tsqan "Ack-{w=0.5}{nw}"
        n 1cllunlsbr "..."
        n 1ccssslsbr "B-{w=0.2}bugs,{w=0.5}{nw}"
        extend 1clrsslsbl " huh?"
        n 1csrunlsbl "..."
        n 1ccsajlsbl "I-{w=0.2}I mean,{w=0.75}{nw}"
        extend 1cllsssbl " don't get me wrong!{w=0.75}{nw}"
        extend 1cllbosbl " Someone's gotta take care of all the creepy crawlies."
        n 1cslemsbl "...I'm just glad it's not me.{w=0.75}{nw}"
        extend 1ccsemsbl " Yeesh."
        n 1csrbosbl "..."
        n 1ccssssbl "W-{w=0.2}well..."
        n 1fllbg "I guess I'll just try not to let it{w=0.5}{nw}"
        extend 1fsqsm " {i}bug me{/i}{w=0.75}{nw}"
        extend 1fwlbg " then!{w=0.75}{nw}"
        extend 1fcssmeme " Ehehe."

        if Natsuki.isLove(higher=True):
            n 1fchblleaf "Love you too,{w=0.2} [player]~!"

    elif persistent._jn_player_pet == "lizards":
        n 1tnmfl "Oh?{w=0.5}{nw}"
        extend 1tllfl " Really,{w=0.2} [player]?{w=0.75}{nw}"
        extend 1tsqem " Lizards?"
        n 1ccsss "Heh.{w=0.75}{nw}"
        extend 1ulraj " I gotta say,{w=0.2} I'm impressed -{w=0.5}{nw}"
        extend 1fchbg " you {i}finally{/i} found a pet just as cold-blooded as you!"
        n 1fsqdv "..."
        n 1fchdvesi "Pfffft!{w=0.75}{nw}"
        extend 1fchbg " I'm kidding,{w=0.2} [player]!{w=0.5} I'm just kidding!{w=0.75}{nw}"
        extend 1fcsss " Jeez..."
        n 1fsqbo "..."
        n 1fnmem "Hey!{w=0.75}{nw}"
        extend 1flrfl " Come on,{w=0.2} [player].{w=0.75}{nw}"
        extend 1fcspo " Don't give me that look.{w=0.75}{nw}"
        extend 1fchgn " It was a good one this time!"
        n 1fcsbg "And besides -{w=0.5}{nw}"
        extend 1unmaj " think about it!"
        n 1fnmsg "Shouldn't you of all people know how to{w=0.5}{nw}" 
        extend 1fsqbg " {i}shed{/i}{w=0.75}{nw}" 
        extend 1fchbg " things off?{w=0.75}{nw}"
        extend 1nchgn " Ahaha."

        if Natsuki.isLove(higher=True):
            n 1fwlbgl "Love you too,{w=0.2} [player]~!"

        else:
            n 1fwlbg "I'm here all week,{w=0.2} [player]!"

    elif persistent._jn_player_pet == "mice":
        n 1tlrbo "Mice,{w=0.2} eh?{w=0.75}{nw}"
        extend 1tsqfl " Are you sure,{w=0.2} [player]?"
        n 1fsqss "Not hamsters?{w=0.75}{nw}"
        extend 1fnmbg " Gerbils?"
        n 1fsqbg "...Or do I smell a rat?"
        n 1fsqsm "..."
        n 1fchsm "Ehehe.{w=0.75}{nw}"
        extend 1ullss " Nah,{w=0.2} I guess I can see the appeal.{w=0.75}{nw}"
        extend 1unmaj " They're social animals,{w=0.2} right?"
        n 1fcssmesm "You better be giving yours plenty of company too,{w=0.2} [player]!"

    elif persistent._jn_player_pet == "rats":
        n 1tnmbo "Rats,{w=0.2} huh?{w=0.75}{nw}"
        extend 1ullaj " That's pretty cool."
        n 1ullsl "..."
        n 1fsqsm "..."
        n 1fcsbg "Oh?{w=0.75}{nw}"
        extend 1fnmbg " What's the matter,{w=0.2} [player]?{w=0.75}{nw}"
        extend 1fnmsm " Were you {i}expecting{/i} me to be grossed out or what?"
        n 1fcssm "Ehehe.{w=0.75}{nw}"
        extend 1tllss " Nah,{w=0.2} rats are fine.{w=1}{nw}"
        extend 1unmaj " They're super smart too!{w=0.75}{nw}"
        extend 1nlrsssbl " But I bet you already knew that by now anyway."
        n 1fcsss "Heh.{w=0.75}{nw}"
        extend 1fcsbg " In fact..."
        n 1fsqbg "I bet you probably have plenty of {i}tails{/i} to tell by now,{w=0.5}{nw}"
        extend 1nchgn " right?"
        ...
        n 1unmbg "Bunnies!{w=0.75}{nw}"
        extend 1uchgnl " Yes!{w=0.75}{nw}"
        extend 1ccsssl " Man..."
        n 1cllbolsbr "..."
        n 1ccsfllsbr "I-{w=0.2}I mean,{w=0.5}{nw}"
        extend 1fcsajlsbr " how could anyone {i}not{/i} love those little fuzzballs?{w=0.75}{nw}"
        extend 1fllsslsbr " Are you kidding me?!"
        n 1fcsbgl "Especially those ones with the floppy ears!{w=0.75}{nw}" 
        extend 1nchgnl " They look so cuddly!"
        n 1fllbg "And then just to top if off,{w=0.5}{nw}" 
        extend 1fspaj " they come in all those different colors too!{w=0.75}{nw}"
        extend 1fchbg " Talk about variety!"
        n 1ullaj "Just make sure you keep that cage spick and span though,{w=0.2} [player] -{w=0.5}{nw}"
        extend 1csqpo " I'm not putting up with any {i}dust{/i} bunnies on my watch."

    elif persistent._jn_player_pet == "snakes":
        n 1unmemesh "H-{w=0.2}huh?{w=0.75}{nw}"
        extend 1ccsemlsbl " N-{w=0.2}now just hold on a second,{w=0.2} [player].{w=0.75}{nw}"
        extend 1clremlsbl " {i}Snakes{/i}?"
        n 1csrunlsb "..."
        n 1fcsunsbr "Uuuuuu..."
        n 1kcsaj "...Fine.{w=0.75}{nw}" 
        extend 1cslbosbr " I guess I'll just be straight with you,{w=0.2} [player]."
        n 1ccsfllsbr " I'm...{w=1}{nw}"
        extend 1ksrsllsbr " not {i}amazing{/i} with those."
        n 1fcsfllsbr "S-{w=0.2}snakes,{w=0.2} I mean."
        n 1fllfll "They just...{w=1}{nw}" 
        extend 1cslcal " don't really agree with me.{w=0.75}{nw}" 
        extend 1cslsll " I don't know why."
        n 1fcsgsl "B-{w=0.2}but that's not to say that they {i}can't{/i} be cute,{w=0.1} obviously!{w=0.75}{nw}"
        extend 1flrpo " Making that kind of assumption is just being ignorant."
        n 1ksrpo "...And they deserve care just like any other pet."

        if Natsuki.isAffectionate(higher=True):
            n 1fcsaj "Besides,{w=0.2} [player] -{w=0.5}{nw}" 
            extend 1ccsca " you know what you signed up for."
            n 1fchgn "You can't {i}shed{/i} your responsibilities that easily!{w=0.75}{nw}"
            extend 1nchgn " Ehehe."

        else:
            n 1flraj "So!{w=0.75}{nw}"
            extend 1fcsposbr " You better not be flaking out on yours,{w=0.2} [player]!"
            n 1fcscaesmsbr "That's all I'm saying!"

    elif persistent._jn_player_pet == "something_else":
        n 1tsqbg "Oho?{w=0.75}{nw}"
        extend 1tnmbg " An exotic owner,{w=0.2} are we?"
        n 1fcsbg "...And why's that,{w=0.2} [player]?{w=0.75}{nw}"
        extend 1fsqss " Regular pets just aren't {i}special{/i} enough for you?{w=0.75}{nw}"
        extend 1fupfl " You just {i}had{/i} to stand out?"
        n 1fsqsm "..."
        n 1fnmsm "Well?{w=0.75}{nw}"
        extend 1fcsbs " Spit it out!"
        n 1fsqcs "..."
        n 1nchgn "Ahaha.{w=0.75}{nw}"
        extend 1tnmaj " In all seriousness though,{w=0.2} [player]?"
        n 1clrca "..."
        n 1ccstr "Just...{w=1}{nw}"
        extend 1csqsl " make sure you know what you're doing.{w=0.75}{nw}"
        extend 1csqaj " Capiche?"

        if Natsuki.isAffectionate(higher=True):
            n 1unmemlsbl "I-{w=0.2}it's not that I think you're automatically a bad owner or anything!{w=0.75}{nw}"
            extend 1fcsemlsbl " Of course not!{w=0.75}{nw}"
            extend 1fcspol " Only a {i}real{/i} dummy just jumps to those kinds of assumptions."
            n 1csrbol "But..."
            n 1ccsajl "I {i}have{/i} heard about how demanding they can be.{w=0.75}{nw}"
            extend 1csqbo " And trust me."
            n 1fcsss "You really don't want {i}my{/i} demands on top of that too."
            n 1fsqsm "Ehehe."

            if Natsuki.isLove(higher=True):
                n 1fchbll "Love you too,{w=0.2} [player]~!"

        else:
            n 1ccsajlsbr "Y-{w=0.2}you owe the animal that much,{w=0.2} after all.{w=0.75}{nw}"
            extend 1fcscalesm " Remember that."
