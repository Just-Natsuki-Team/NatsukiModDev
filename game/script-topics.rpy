...
label talk_did_you_have_pets:
    ...
    if persistent._jn_player_pet == "birds":
        ...
        if Natsuki.isAffectionate(higher=True):
            n 1unmemlsbl "I-{w=0.2}it's not that I think you're automatically a bad owner or anything!\n{w=0.75}{nw}"
            extend 2fcsemlsbl "Of course not!"
    ...
    if isinstance(joke_choice, jn_jokes.JNJoke):
        ...
        if joke_choice.joke_category == jn_jokes.JNJokeCategories.funny:
            ...
            if dialogue_choice == 1:
                ...
                extend 1cllbo " I wouldn't complain if {i}that{/i} one never showed up again.\n{w=0.75}{nw}"
