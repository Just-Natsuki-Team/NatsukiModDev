...
label talk_can_you_draw:
    ...
    if Natsuki.isNormal(higher=True):
        if already_discussed_topic:
            n  "Wait...{w=1}{nw}"
            extend  " drawing?{w=0.75}{nw}"
            extend  " I could have sworn we talked about this before,{w=0.2} [player]...{w=1}{nw}"
            extend  " huh."
            n  "Well,{w=0.2} anyway."
            n  "I'm still kinda peeved about how my school handled art classes.{w=0.75}{nw}"
            extend  " Like yeah,{w=0.2} we had them...{w=1}{nw}"
            extend  " But they weren't exactly super in-depth or anything.{w=0.75}{nw}"

        else:
            n  "Huh?{w=0.75}{nw}"
            extend  " Can I draw?{w=0.75}{nw}"
            extend  " Like...{w=0.3} portraits,{w=0.2} landscapes,{w=0.5}{nw}" 
            extend  " that kind of thing?"
            n  "Heh.{w=0.75}{nw}"
            extend  " I wish!{w=0.75}{nw}"
            extend  " And believe me -{w=0.5}{nw}"
            extend  " you'd know about it if I had {i}that{/i} kind of talent."
            n  "I mean,{w=0.2} have you even {i}seen{/i} the sort of stuff people post online now?"
            n  "It's insane!{w=0.75}{nw}"
            extend  " I have no idea how they do it!"
            n  "...Or even where they start,{w=0.2} for that matter."
            n  "We had art class during school,{w=0.5}{nw}"
            extend  " obviously.{w=0.75}{nw}"
            extend  " But it wasn't really super in-depth or anything.{w=0.75}{nw}"

        extend  " Like...{w=0.3} at all."
        n  "In fact.{w=0.75}{nw}"
        extend  " the more I think about it..."
        n  "It hardly even {i}qualified{/i} as art class at all!{w=0.75}{nw}"
        extend  " It felt like we barely broke out the canvas!"
        n  "I swear -{w=0.5}{nw}"
        extend  " so many lessons were just an exercise in page turning more than {i}painting{/i}."
        n  "I get that art has a lot of history behind it.{w=0.75}{nw}"
        extend  " And of course there's tons of styles out there to learn.{w=0.75}{nw}"
        extend  " I get it."
        n  "But come{w=0.5}{nw}" 
        extend  " {b}on{/b}!{w=1}{nw}"
        extend  " What's even the point in studying so much about all these different artists and styles..."
        n  "...If we barely even had the time to cover any of the basics?{w=0.75}{nw}"
        extend  " Let alone try any of the stuff we studied out!"
        n  "A-{w=0.2}and besides!{w=0.75}{nw}"
        extend  " All those artists already had their turn.{w=0.75}{nw}"
        ...
        n  "..."
        n  "I guess it wouldn't have been so bad if I could have just practiced outside school.{w=0.5}{nw}"
        ...
        extend  " Honestly?"
        n  "It's not that I wouldn't want to be able to draw cool stuff.{w=0.75}{nw}"
        extend  " Are you kidding?{w=0.75}{nw}"
        ...
        if Natsuki.isAffectionate(higher=True):
            ...
            n  "..."
            n  "It's...{w=1}{nw}" 
            extend  " intimidating.{w=1}{nw}"
            extend  " Trying to teach yourself something like that."
            n  "It's fine if you've always been drawing,{w=0.2} or you have some kind of teacher."
            n  "...But what if you don't {i}have{/i} that kind of direction anywhere?"

            if Natsuki.isEnamored(higher=True):
                n  "People always say it's just down to practice,{w=0.75}{nw}"
                extend  " or to look up some random course online or whatever.{w=0.75}{nw}"
                extend  " A-{w=0.2}and I'm not saying they're wrong!"
                n  "Everyone has their own approach,{w=0.2} obviously."
                n  "...I guess I just never found mine.{w=0.75}{nw}"
                extend  " Not {i}yet{/i},{w=0.2} anyway."
                n  "Though...{w=1}{nw}"
                ...
                extend  " I'm kinda bored of listening to myself now.{w=0.75}{nw}"
                extend  " So what about you,{w=0.2} [player]?"
            ...
            n  "Besides.{w=0.2} I think I'm about done listening to myself talk for now."
            n  "So...{w=0.75}{nw}"
            extend  " what about you,{w=0.2} [player]?"
        ...
        if jn_activity.ACTIVITY_MANAGER.hasPlayerDoneActivity(jn_activity.JNActivities.artwork):
            ...
            extend  " I've got a pretty good hunch for this kind of thing,{w=0.5}{nw}"
            extend  " if I say so myself."
            n  "Ehehe."
            n  "Yep!"
            n  "You've got 'artist' written all over you."
            n  "All the more reason to be grateful you've always got such a great subject on hand,{w=0.5}{nw}" 
            extend  " am I right?"
            ...
            menu:
                ...
                "Yes,{w=0.2} I draw often!":
                    n  "Oh?{w=0.75}{nw}"
                    extend  " Fancy yourself an artist,{w=0.2} huh?"
                    n  "Ehehe."
                    n  "Well,{w=0.5}{nw}"
                    extend  " you better consider yourself lucky,{w=0.2} [player].{w=0.75}{nw}"
                    extend  " Not every day such an amazing subject just plonks itself down right in front of you!"

                "No,{w=0.2} I don't usually draw.":
                    n  "Really?{w=0.75}{nw}"
                    extend  " Aww..."
                    n  "I'll admit,{w=0.2} I am kinda disappointed.{w=1}{nw}"
                    extend  " I get it though.{w=0.75}{nw}"
                    extend  " Not like {i}I'm{/i} one to judge for that anyway."

                "I don't draw anymore.":
                    n  "Aww..."
                    n  "..."
                    n  "...Wait.{w=0.2} 'Anymore'?{w=0.75}{nw}"
                    extend  " What do you mean,{w=0.2} {i}'anymore'{/i}?{w=0.5}{nw}"
                    extend  " You gotta get back into it,{w=0.2} [player]!"
                    n  "Why not?{w=0.75}{nw}"
                    extend  " {i}Especially{/i} when you've been blessed with {i}such{/i} a good subject to get started!{w=0.75}{nw}"
                    extend  " Ehehe."

        n  "Well anyway.{w=0.75}{nw}"
        extend  " You know what?{w=0.75}{nw}"
        extend  " Who's to say I couldn't learn to draw at some point?"
        n  "There's gotta be some art supplies I can steal from somewhere around here."
        n  "Not like the art club's gonna need 'em.{w=0.75}{nw}"
        extend  " And who knows,{w=0.2} [player]?"
        n  "I bet there's a Picasso in me yet!"
        n  "I just gotta{w=0.5}{nw}"
        extend  " {i}ease-l{/i}{w=0.5}{nw}"
        extend  " into it first!{w=0.75}{nw}"
        extend  " Ehehe."
        ...
        if already_discussed_topic:
            n  "..."
            n  "No,{w=0.2} [player].{w=1}{nw}"
            extend  " Like I {i}already{/i} told you.{w=0.75}{nw}"
            extend  " I can't."
            n  "What did you expect?{w=0.75}{nw}"
            extend  " I already said how my art classes sucked,{w=0.75}{nw}"
            extend  " if you were even paying attention."

        else:
            n  "No,{w=0.2} [player].{w=0.75}{nw}"
            extend  " I can't.{w=0.75}{nw}"
            extend  " If that really matters to you."
            n  "And not because I never wanted to learn or anything like that either."
            n  "You'd think art class of all places would be the best place to try and pick it up."

        n  "We barely even got out the art supplies -{w=0.75}{nw}"
        extend  " we spent way more time learning about how {i}other people{/i} worked than actually learning ourselves!"
        n  "It was just a complete disappointment."
        n  "..."
        n  "Look.{w=0.2} I'm not dumb.{w=1}{nw}"
        extend  " I get that you gotta learn about why people do art,{w=0.75}{nw}"
        extend  " or all the different styles and how they work."
        n  "But come{w=0.5}{nw}"
        extend  " {b}on{b}!{w=0.75}{nw}"
        extend  " It's not like the practical part is any less important."
        n  "I wouldn't even care so much if we'd at least covered all of the basics {i}before{/i} dusting off all the stupid textbooks.{w=0.75}{nw}"
        extend  " Not all of us have the time for an art club."
        n  "...Or the luxury of practicing at home."
        n  "Heh.{w=0.75}{nw}"
        extend  " I would say there's not much stopping me from trying to learn now.{w=0.75}{nw}"
        extend  " Not like spare time is a problem anymore."
        n  "But honestly?"
        n  "{i}Something{/i} tells me I'm not gonna be in the mood for that a whole lot."
        n  "I wonder why..."
        extend  " {i}[player]{/i}."
        ...
        if already_discussed_topic:
            n  "...Seriously?{w=1}{nw}"
            extend  " What am I,{w=0.2} a broken record?"
            n  "The only thing I'm drawing right now is an end to this {i}stupid{/i} conversation."
            n  "Jerk."

        else:
            n  "...Heh.{w=0.75}{nw}"
            extend  " Can I draw?{w=1}{nw}"
            extend  " Really?"
            n  "..."
            n  "...Actually.{w=0.75}{nw}"
            extend  " You know what?"
            n  "Yeah.{w=0.2} I am.{w=0.75}{nw}"
            extend  " At drawing conclusions."
            n  "...And I've got plenty drawn about the likes of {b}you{/b}."
